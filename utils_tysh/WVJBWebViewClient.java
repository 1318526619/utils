...
import com.wbg.tianyi_sj.activity.ShopInfoActivity;

@SuppressLint({"SetJavaScriptEnabled", "NewApi"})
public class WVJBWebViewClient extends WebViewClient {

    private static final String kTag = LogUtil.TAG + ":WVJB";
    ...
    private void processQueueMessage(String messageQueueString) {
        Log.i(kTag, "the html data is :\n" + messageQueueString);
        String replace = messageQueueString.replaceAll("\"\\{", "\\{").replaceAll("\\}\"", "\\}");
        Log.i(kTag, "replace is the :\n" + replace);
        ShopInfoActivity.locationAddress(replace);
        //下面的先不用管
        try {
            JSONArray messages = new JSONArray(replace);
            for (int i = 0; i < messages.length(); i++) {
                JSONObject jo = messages.getJSONObject(i);
//                log(kTag, jo);
                JSONObject dataJson = jo.getJSONObject("data");
//                Log.i(kTag, "resulet is :" + dataJson);

                WVJBMessage message = JSONObject2WVJBMessage(dataJson);
                if (message.responseId != null) {
